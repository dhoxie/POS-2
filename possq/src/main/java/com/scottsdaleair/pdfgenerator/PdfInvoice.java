...
/**
 * Class to create a PDF for a given invoice.
...
public class PdfInvoice {
  ...
  /**
   * Create a PdfInvoice from invoice for use in creating a pdf.
   * @param theInvoice invoice to be used
   */
  ...
  public PdfInvoice start() throws FileNotFoundException {
    ...
    PdfWriter writer = new PdfWriter(theInvoice.getId()
        + theCust.getFname() + theCust.getLname() + ".pdf");
    ...
    for (int i = 0; i < theInvoice.getParts()
        .length; i++) {
  ...
  /**
   * creates part that says partNum Description and price info.
   * 
   * @return is for junit
   */
  ...
  public Document addNotesHeader() {
    ...
    Cell topCell = new Cell(1, 1)
        ...
        .setBorderRight(Border.NO_BORDER);
    bTable.addCell(topCell);
    theDocument.add(new Paragraph("Notes")
        .setFontSize(8));
    theDocument.add(bTable);
    return theDocument;
  }
  ...
  public Document addPart(Part toAdd) {
    ...
    pTable.addCell(new Cell(1, 1)
        .add(new Paragraph(toAdd.getPartNum())));
    pTable.addCell(new Cell(1, 1)
        .add(new Paragraph(toAdd.getVendor())));
    pTable.addCell(new Cell(1, 1)
        .add(new Paragraph("inc")));
    pTable.addCell(new Cell(1, 1)
        .add(new Paragraph(toAdd.getPrice())));
    pTable.addCell(new Cell(1, 1)
        .add(new Paragraph("disco")));
    pTable.addCell(new Cell(1, 1)
        .add(new Paragraph("total")));
