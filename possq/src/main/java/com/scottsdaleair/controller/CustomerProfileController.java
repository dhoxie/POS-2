...
public class CustomerProfileController {
  ...
  public void loadData(Customer cust) {
    ...
    if (phones != null) {
      for (PhoneNumber phone : phones) {
        if (phone.getName().equals("home")) {
          txtHomePhone.setText(phone.getNum());
        }
        if (phone.getName().equals("cel")) {
          txtCellPhone.setText(phone.getNum());
        }
        if (phone.getName().equals("work")) {
          txtWorkPhone.setText(phone.getNum());
        }
      }
    }
  ...
  private void viewVehicle(TableRow<Vehicle> row) throws IOException {
    ...
    FXMLLoader loader = new FXMLLoader(
        new File("src/main/java/com/scottsdaleair/view/Vehicle_Profile_Screen.fxml").toURI().toURL());
