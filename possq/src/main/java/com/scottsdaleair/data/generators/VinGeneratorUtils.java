...
/**
 * Utility class providing help methods to generate vehicle identification
 * number (VIN).
 */
public final class VinGeneratorUtils {
  ...
  public static String getRandomVin() {
    ...
    try {
      ...
      // should never happen, because here we calculate checksum for VIN generated by
      // us
  ...
  private static Vin getRandomVinStart() {
    ...
    try (final Stream<String> stream = Files.lines(Paths.get("./" + PREFIXES_FILE_NAME))) {
      final String line = stream.skip(lineToRead).findFirst().orElseThrow(
          () -> new RuntimeException("Problem occurred while read line "
          + lineToRead + " from " + PREFIXES_FILE_NAME));
  ...
  private static class Vin {
    ...
    Vin(final String wmi, final String vds, final String year) {
      this.wmi = wmi;
      this.vds = vds;
      this.year = year;
    }
